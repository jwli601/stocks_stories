...
import akshare as ak # akshare ÓÃÀ´¶ÁÈ¡µ±ÈÕµ±Ê±µÄÊý¾Ý£¬akshareÈç¹ûÓÃÀ´¶ÁÀúÊ·Êý¾Ý¾­³£»á³öÏÖÊý¾Ý¶ªÊ§ÏÖÏó£¬²»ºÃÓÃ
...
def sw_heatmap():
    ...
    sw_1st_list = [i[:6]for i in sw_1st_info['ÐÐÒµ´úÂë']]
    sw_data = pd.DataFrame(columns=sw_1st_list)
    for i in sw_1st_list:
        sw_data[i] = ak.index_hist_sw(i).set_index('ÈÕÆÚ')['ÊÕÅÌ']

    sw_today = ak.index_realtime_sw('Ò»¼¶ÐÐÒµ').set_index('Ö¸Êý´úÂë')
    sw_data.loc[date_today,:] = sw_today['×îÐÂ¼Û']
    sw_data.columns = sw_1st_info['ÐÐÒµÃû³Æ']
...
def zh_us_interests_diff(start_date=''):
    ...
    bond_table.set_index('ÈÕÆÚ',inplace=True)
    bond_table.index = [str(i) for i in bond_table.index]
    bond_table['ÖÐÃÀÀû²î_10Y'] = bond_table['ÖÐ¹ú¹úÕ®ÊÕÒæÂÊ10Äê']-bond_table['ÃÀ¹ú¹úÕ®ÊÕÒæÂÊ10Äê']
    zmlc_10Y = bond_table['ÖÐÃÀÀû²î_10Y'].dropna()
    ...
    plt.title('ÖÐÃÀÀû²îÓëÉÏÖ¤50Ö¸Êý×ßÊÆ corr = {}'.format(round(corr,3)))


    plt.plot(df1_1.z,color='red',label='ÖÐÃÀÀû²î')
    plt.plot(df1_1.sz50,color='blue',alpha=0.2,label='ÉÏÖ¤50\n  corr={}'.format(round(corr,3)))
...
def stock_bond_rolling(start_date=''):
    ...
    plt.legend()
    plt.title('¹ÉÕ®ÂÖ¶¯Ä£ÐÍ corr = {}'.format(round(df_rocz.corr().loc['sz50','bond_index'],3)))
    plt.show()
    return df
def sz50_over_zz500(start_date = ''):
    ...
    plt.plot(df.sz50,alpha=.4,label='ÉÏÖ¤50')
    plt.plot(df.zz500,alpha=.4,label='ÖÐÖ¤500')
    # plt.plot(df['sz50/zz500'],color='green',label='ÉÏÖ¤50/ÖÐÖ¤500')
    plt.xticks(df.index[::60],rotation=90)
    plt.title('¹æÄ£Ð§Ó¦£ºÉÏÖ¤50/ÖÐÖ¤500')
    plt.legend()
    plt.show()
...
def hs300_streamline():
    ...
    hs300_list = [i[:6] for i in hs300_list['Ö¤È¯´úÂë']]
    ...
    for j in range(1,5):
        ...
        for i in range(len(a1)):
            ...
            ax.set_title('ÏòµÚ{}ÏóÏÞÒÆ¶¯:{}Ö»'.format(j,len(a1)))
def run_all():
    sw_heatmap()
